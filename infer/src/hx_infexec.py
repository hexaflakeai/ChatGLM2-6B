...
from hxinfer import HxInfer


infer_lib = cdll.LoadLibrary("libhxinfer.so")
...
class GLMInfer(GenerationMixin):
    def __init__(self, engine_file, **kwargs):
        ...
        self.split_stragety = (
            0 if kwargs.get("split_stragety") is None else kwargs["split_stragety"]
        )
        self.config_file = (
        ...
        self.inf_obj = HxInfer(
            ...
            split_stragety=self.split_stragety,
    ...
    def __deal_use_cache_output(self, model_idx):
        ...
        for job in range(self.batch_size):
            ...
            if not self.finish_flag_[job]:
                ...
                next_token = np.array(self.next_input[job])
    ...
    def __infer_base(self, inputs):
        ...
        for r in range(self.base_round):
            for i in range(len(inputs)):
                ...
                pos_ = pos[:, base : base + self.base_length].transpose([1, 0])
                ...
                self.inf_obj.set_input(i, "pos_t", pos_)
    ...
    def __get_mask(self, job, token_idx):
        align = self.base_round * self.base_length
        mask = self.attention_mask[job][
            :, :, align + token_idx : align + token_idx + 1, :
        ]
        return mask
    ...
    def __set_binding_inc(self, token_idx):
        ...
        for job_index in range(self.batch_size):
            ...
            if token_idx == 0:
                self.inc_pos = self.pos_[job_index][:, -1:].transpose([1, 0])
                self.inc_pos += 1
            else:
                pos_mask = np.array(self.next_input[job_index]).reshape(1, -1) != 2
                self.inc_pos[pos_mask] += 1
            pos = np.array(self.inc_pos, dtype="int32")

            self.inf_obj.set_input(job_index, "pos_t", pos, model_idx)
    ...
    def get_next_token(self, next_token_logits, dynamic_batch_idx):
        ...
        next_token_scores = self.logits_warper(t_input_ids, next_token_scores)
        if self.generation_config.do_sample:
            ...
            next_token = np.argmax(next_token_scores.numpy(), axis=-1)
        return next_token
    ...
    def stop_inference(self, next_token, dynamic_batch_idx):
        ...
        unfinished_sequences = unfinished_sequences.mul(
            torch.Tensor((sum(next_token != i for i in self.eos_token_id))).long()
        )
